...
def usage():
    print(
        "Usage: ./topology/generate_binary_topo.py [num_layers]\n\t"
        "Generate the Binary Tree topology config file `topology/p4app_binary.json` "
        "with [num_layers] layers"
    )
...
try:
    ...
    print(f"Failed to parse the argument [num_layers]! Cause: {e}")
    usage()
    exit(1)
...
if I == 4:
    ...
    usage()
    sys.exit(1)
print(f"There are {I} layers in the Binary topology")
...
for layer_id in range(I):
    ...
    for device_id in range(device_num):
        links += '["%s%d", "%s%d", {"bw": %d}], ' % (
            device_symbol[layer_id],
            device_id + 1,
            device_symbol[layer_id + 1],
            device_id * 2 + 1,
            link_bw[layer_id],
        )
        links += '["%s%d", "%s%d", {"bw": %d}], ' % (
...
try:
    ...
    print(
        "Failed to open file topology/p4app_binary.json to write the JSON config! "
        f"Cause: {e}"
    )
try:
    f.write(template % (links, hosts, switches))
except Exception as e:
    print(f"Failed to write to file topology/p4app_binary.json! Cause: {e}")
